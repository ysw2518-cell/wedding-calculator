...
export default function GuestTab({ guests, onChange }: Props) {
  ...
  const add = () => {
    ...
    onChange([
      ...guests,
      { ...form, amount: amt, id: crypto.randomUUID() },
    ]);
        ...
        <div className="flex flex-wrap gap-2">
          {AMOUNTS.map((a) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                form.amount === a && !customAmt
                  ? "bg-[#C5A050] text-white"
                  : "bg-[#F3F0E4] text-[#8FAA90]"
              }`}
            >
              {fmt(a)}
        ...
        <>
          <div className="flex gap-2">
            {(["all", "groom", "bride"] as const).map((f) => (
              ...
              >
                {f === "all" ? `전체 ${guests.length}명` : f === "groom" ? `🤵 ${guests.filter((g) => g.side === "groom").length}명` : `👰 ${guests.filter((g) => g.side === "bride").length}명`}
              </button>
            ))}
          </div>
            ...
            {filtered.map((g) => (
              ...
              >
                <div
                  ...
                  style={{ backgroundColor: g.side === "groom" ? "#2E5E38" : "#9A7095" }}
                />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    <p className="font-bold text-sm text-[#2A3C2C] truncate">{g.name}</p>
                    <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-[#F3F0E4] text-[#8FAA90]">
                      {g.relation}
                    </span>
                  </div>
...
function StatBox({
  label, color, cnt, total,
}: {
  label: string; color: string; cnt: number; total: number;
}) {
  return (
    <div
