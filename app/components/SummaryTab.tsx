  ...
  return (
    <div className="space-y-5">
      ...
      <div
        ...
        style={{ background: "linear-gradient(135deg, #2E5E38 0%, #5E9E65 60%, #C5A050 100%)" }}
      >
        <p className="text-sm opacity-75 mb-1">{surplus ? "🎉 흑자" : "💸 적자"}</p>
        <p className="text-4xl font-bold tracking-tight">
          {netTotal >= 0 ? "+" : ""}{fmt(netTotal)}원
        </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="mt-3 space-y-2 text-sm">
            ...
            {expenses.filter((e) => e.side === "groom").length > 0 && (
              <>
                ...
                {expenses.filter((e) => e.side === "groom").map((e) => (
            ...
            {expenses.filter((e) => e.side === "bride").length > 0 && (
              <>
                ...
                {expenses.filter((e) => e.side === "bride").map((e) => (
                  <ExpRow key={e.id} label={`· ${e.label}`} amt={e.amount} />
                ))}
              </>
            )}
        ...
        <div className="bg-white rounded-2xl border border-[#CCE5CC] shadow-sm p-4">
          ...
          <div className="mt-3 space-y-3">
            <SideCard
              label="🤵 신랑측" color="#2E5E38"
              gift={groomGift} meal={groomMeal} venue={groomVenue}
              extra={groomExtras} expense={groomExpense} net={groomNet}
            />
            <SideCard
              label="👰 신부측" color="#9A7095"
              gift={brideGift} meal={brideMeal} venue={brideVenue}
              extra={brideExtras} expense={brideExpense} net={brideNet}
            />
          </div>
        </div>
      </div>
...
function GiftRow({ label, color, amt, cnt, bold }: { label: string; color: string; amt: number; cnt: number; bold?: boolean }) {
  return (
    <div className="flex items-center gap-3">
      <div className="w-1 h-9 rounded-full flex-shrink-0" style={{ backgroundColor: color }} />
      <span className={`text-sm flex-1 ${bold ? "font-bold" : "font-medium"}`} style={{ color }}>
        {label}
      </span>
      <span className="text-xs text-[#8FAA90]">{cnt}명</span>
      <span className="text-base font-bold" style={{ color }}>{fmt(amt)}원</span>
    </div>
  );
}
...
function SideCard({ label, color, gift, meal, venue, extra, expense, net }: {
    ...
    <div className="rounded-2xl p-4 border" style={{ backgroundColor: `${color}0d`, borderColor: `${color}30` }}>
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="text-right">
          ...
          <p className="text-lg font-bold" style={{ color: isPlus ? "#27AE60" : "#E74C3C" }}>
            {isPlus ? "+" : ""}{fmt(net)}원
          </p>
        </div>
      </div>
      <div className="flex gap-2 mb-3">
        ...
        <div className="flex-1 rounded-xl p-2" style={{ backgroundColor: `${color}1a` }}>
          <p className="text-[10px]" style={{ color: `${color}aa` }}>💸 부담</p>
          <p className="text-sm font-bold" style={{ color }}>{fmt(expense)}원</p>
        </div>
      </div>
