...
export default function ExpenseTab({ expenses, onChange }: Props) {
  ...
  const add = () => {
    ...
    onChange([...expenses, { id: crypto.randomUUID(), label: label.trim(), amount: amt, side }]);
    setLabel("");
    setAmount("");
  };
  ...
  const groomTotal = expenses.filter((e) => e.side === "groom").reduce((s, e) => s + e.amount, 0);
  const brideTotal = expenses.filter((e) => e.side === "bride").reduce((s, e) => s + e.amount, 0);
      ...
      <div>
        {expenses.length > 0 ? (
          <div className="space-y-2 lg:max-h-[calc(100vh-280px)] lg:overflow-y-auto">
            {expenses.map((e) => (
              <div key={e.id} className="bg-white rounded-xl border border-[#CCE5CC] p-3 flex items-center gap-3">
                <div className="w-1.5 h-10 rounded-full flex-shrink-0" style={{ backgroundColor: e.side === "groom" ? "#2E5E38" : "#9A7095" }} />
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-sm text-[#2A3C2C] truncate">{e.label}</p>
                  <p className="text-xs text-[#8FAA90]">{e.side === "groom" ? "🤵 신랑측" : "👰 신부측"}</p>
                </div>
                <p className="font-bold text-sm text-[#2A3C2C]">{fmt(e.amount)}원</p>
                <button onClick={() => remove(e.id)} className="text-[#CCE5CC] hover:text-red-400 text-lg leading-none ml-1">×</button>
              </div>
            ))}
          </div>
        ) : (
